...
var model = {
    ...
    serialize: function () {
        ...
        if (_.isArray(schema.rangeKeyAttributes) && schema.rangeKeyAttributes.length > 1) {
...
var Model = {
    ...
    findOne: function (attrs, callback) {
        ...
        if (!schema.hasRangeAttributes() && attrs[schema.hashKey]) {
            params = {
        ...
        self.find({ match: attrs }, function(err, collection, units){
            if (err) { return self.protectedCallback(callback,err) }; //_.isNumber(err.statusCode) && err.statusCode!=200
            ...
            self.protectedCallback(callback,null, collection.models[0], units);
        });
    },
    ...
    find: function (options, callback) {
        ...
        self.connection.client.query(query, function(err, data){
            if (err) { // _.isNumber(err.statusCode) && err.statusCode!=200
                return self.protectedCallback(callback,err,data);
            }
            if (data.Items && _.isNumber(options.skip)) {
                try
                {
                    data.Items = data.Items.slice(options.skip);
                }
                catch (err)
                {
                    return self.protectedCallback(callback,err);
                }
            }
            return self.protectedCallback(callback,err, collection(_.map(data.Items, self.parse, self)), data.ConsumedCapacity.CapacityUnits);
        });
        
    },

    protectedCallback: function (callback,err,data, units) { // self.connection.client.query is running crazy for callback-side errors
        try{
