;(function communityControllerEnclosure() {
  'use strict';
  ...
  function archive(authUser, communityId, db, callback) {
    sergeant({
      ...
      community: {
        ...
        load: communityId,
        after: sergeant.stopIfNotFound,
        beforeSave: prepCommunityArchive.bind(null, db),
        save: true,
  ...
  function prepCommunityArchive(db, data) {
  ...
  function get(optionalUser, communityId, db, callback) {
    var tasks = {
      ...
      founder: {
        table: db.membership,
        json: true,
        before: getSetFounderIdFromCommunity,
        finally: sergeant.minimalJson }
    };
  ...
  function list(optionalUser, db, callback) {
    var tasks = {
      ...
      memberships: {
        table: db.membership,
        multiple: {},
        finally: sergeant.jsonMap.bind(null, 'communityId') }
    };
  ...
  function filterSecretCommunities(db, callback, data) {
    var communities = data.communities || [],
        ...
        secretCommunity = db.community.model.type.secret,
        keep = [];
  ...
  function listByTag(optionalUser, tags, db, callback) {
    ...
    sergeant(tasks,
            'tags,communities,memberships',
            filterSecretCommunities.bind(null, db, listByTagOrder.bind(null, callback)));
  }
  ...
  function listTop(optionalUser, count, page, db, callback) {
    ...
    if (count > 0) {
      tasks.communities.multiple.limit = count;

      if (page > 0) {
        tasks.communities.multiple.offset = (page - 1) * count;
      }
    }
  ...
  function add(authUser,  community, communityImage, founder, founderImage, files, db, callback) {
    ...
    if (!founder || founder.alias === undefined) {
      callback(Errors.missingInput('founder.alias'));
    ...
    tasks = {
      existingCommunity: {
        table: db.community,
        load: { name: community.name },
        after: sergeant.stopIfFound,
        finally: sergeant.remove },
      communityWithoutFounder: {
        table: db.community,
        data: community,
        save: true,
        finally: sergeant.remove },
      founder: { table: db.membership,
        data: founder,
        before: addSetFounderCommunityId,
        save: true,
        finally: sergeant.json },
      community: {
        table: db.community,
        data: community,
        before: addUpdateCommunityFounder,
        save: true,
        finally: sergeant.json },
      tags: {
        table: db.communityTag,
        data: [],
        beforeSave: prepTags.bind(null, db),
        multiple: {},
        save: true,
        finally: sergeant.remove },
  ...
  function update(authUser,  community, communityImage, founder, founderImage, files, db, callback) {
    sergeant({
      ...
      community: {
        table: db.community,
        before: prepCommunityUpdate.bind(null, community),
        data: {},
        save: true,
        finally: sergeant.json },
      tags: {
  ...
  function prepSetFounder(founderNewDetails, data, tasks) {
    ...
    if (founderNewDetails !== undefined) {
      if (founder.id !== founderNewDetails.id) {
        return (Errors.noPermissions('update-community'));
      } else if (founderNewDetails.alias || founderNewDetails.description) {
