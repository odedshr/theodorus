;(function membershipControllerEnclosure() {
  ...
  function set (authUser, membership, membershipImage, communityId, membershipId, files, db, callback) {
    ...
    var tasks  ={
      current : { table:db.membership, load:{ userId: authUser.id },
        finally: sergeant.remove },
      sameName : { table:db.membership, finally: sergeant.remove },
      community: { table:db.community, load: membership.communityId, after: sergeant.stopIfNotFound,
        finally: sergeant.remove },
      user: { table:db.user, load: authUser.id,
        finally: sergeant.remove },
    ...
    if (membership.name !== undefined) {
      ...
      tasks.sameName.load = { name : membership.name, communityId: membership.communityId };
  ...
  function checkIfSameNameIsCurrent (data) {
    if ((data.sameName !== null) &&
        (data.current === null || data.current.id !== data.sameName.id)){
