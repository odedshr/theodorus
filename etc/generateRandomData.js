(function generateRandomDataClosure () {
  ...
  function randomText (maxLength) {
    for (var acc = [], i = 0, length = 1 + Math.floor(Math.random() * maxLength); i < length; i++) {
      acc.push(words[Math.floor(Math.random() * wordCount)]);
    }
    return acc.join(' ');
  }
  ...
  function gotMemberships (user, data) {
    var memberships = data.memberships, communities = [];
    user.memberships = memberships;
    user.communities = communities;
    for (var i= 0, length = data.memberships.length; i < length; i++) {
      communities[memberships[i].communityId] = true;
    }
    readyForAction(user);
  }
  ...
  function readyForAction (user) {
    ...
    if (randomActionCode < membershipActionsWeight) {
      ...
      if (!user.communities[communityId] &&  randomActionCode < communityActionWeight) {
  ...
  function joinedCommunity (user, data) {
    if (data.membership) {
      ...
      user.communities[data.membership.communityId] = true;
      console.log('joined community ');
      actionInCommunity(user,data.membership);
    } else {
      console.log('failed to join community - ' + data.message);
      nextAction();
    }
  }
  ...
  function topicAdded(community, data) {
    if (data.message) {
      console.log(data.message + ' ' + data.details.value);
    } else {
      data.topic.Opinions = [];
  ...
  function nextAction () {
    if (actionCount--) {
      ...
      for (var i = 0, length = communityIndex.length; i < length; i++) {
